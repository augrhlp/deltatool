...
;********************
FUNCTION GroupByStat::getTitle

 return, 'Daily stats'

END

FUNCTION GroupByStat::getDisplayNames
...
FUNCTION GroupByStat::getValues
...
FUNCTION GroupByStat::getOperations
...
PRO GroupByStat::fillDataFromFile, fileName
...
;		print, bufferString
		...
		if n_elements(info) eq 4 then begin
			thisGroupByStat=getFMGroupByStat()
			thisGroupByStat.code=fix(info[0])
 			thisGroupByStat.displayName=info[1]
			thisGroupByStat.value=info[2]
			thisGroupByStat.operation=info[3]
			groupByStats=[groupByStats, thisGroupByStat]
		endif else begin
 ...
 self.list=ptr_new(groupByStats[1:*], /NO_COPY)

END

FUNCTION GroupByStat::init
...
PRO GroupByStat::cleanUp

 self -> ConfigurableData::cleanUp

END

PRO GroupByStat__Define
