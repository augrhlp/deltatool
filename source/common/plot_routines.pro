...
;  ;MM summer 2012 End
; MM spring 2015
; 1)A new font strategy was integrated in delta (only for graphic devices)
; please use: setUserFont, fontName to set a font BEFORE use it in a IDL routine
; i.e.: xyouts, plot...
; use: setUserFont, /RESET to reset font setting at the idl starting value(s)
; use: myFontName=getUserFont() to get currently user defined font
; font definitions are in the file: C:\Users\Public\Documents\JRC_DELTA\delta\configuration)font.dat
; You MUST set at least:
; 1;LegendFont;0;JasmineUPC;32;1.30000;2.00000
; 2;MainTitleFont;0;JasmineUPC;32;1.30000;2.00000
; 101;PSFont;-1;N/A;N/A;1.50000;1.00000
; Obviously you can(must) change font properties accordingly to platform/graphic behaviour
; NB: when you call setUserFont delta change the followings:
; !P.font
; !P.charsize
; !P.charThick
; and build the requested font (with requested name and truetype properties) calling in the right way: 
; device,set_font=fontString, TT_FONT=TT_FONT
; You can manually modify charsize and charthick if you're in "vector mode" as usual in xyouts, plot... procedures
; 2a)A new method was implemented for result object
; Result->setMultipleDrawMainTitle, yourTitle
; it is useful when you launch a multiple raster image compositions and you want to fill
; the upper header title.
; 2b)To convert ps benchmark files to raster benchmark files
; _in each involved *.rqs file change line
; PlotDeviceName=PS to PlotDeviceName=IMAGE+BMP (Or a valid
; _in (C:\Users\Public\Documents\JRC_DELTA\delta\resource\)resource.ini
; RASTER_FORMAT=BMP (Or a valid idl one such as JPEG, TIFF... accordingly to previous setting in PlotDeviceName)
; this setting OVERRIDE the previous one only when you are in multiple/composite images format mode (to work around different formats settings)
; MM spring 2015 end
...
;    barnames=strmid(longBarNames1,0,2)+strmid(longBarNames1,4,2)
    ...
    if nchlp ge 1 then begin
      ;setUserFont, 'UserDef1', FORCELOG=FORCELOG
  ...
  for isubbar=0,nsubbars-1 do begin
    if nsubbars ge 2 then begin
      ...
      for idot=0,nDots -1 do begin
        ...
        if nchlp ge 1 then begin
          ;April 2015 MM look @ configuration/font.dat for user def fonts
          ;setUserFont, 'UserDef1', FORCELOG=FORCELOG
          if plotter->currentDeviceIsPostscript() then setUserFont, 'PSFont', FORCELOG=FORCELOG else setUserFont, 'LegendFont', FORCELOG=FORCELOG
          xyouts,tickV(chlp),float(replicate('1.',nchlp))*recognizeRangeY,$
            replicate('',nchlp),orientation=90
        ;          xyouts,tickV(chlp),float(replicate('1.',nchlp))*recognizeRangeY,$
...
PRO FM_PlotBarsLegend, plotter, request, result
  ...
  ;setUserFont, 'MediumFont', FORCELOG=FORCELOG
  if plotter->currentDeviceIsPostscript() then setUserFont, 'PSFont', FORCELOG=FORCELOG else setUserFont, 'LegendFont', FORCELOG=FORCELOG
  targetInfo=result->getGenericPlotInfo()
  allDataXY=targetInfo->getXYS()
...
PRO FM_PlotDynamicEvaluation, plotter,request,result
  ...
  if elabCode ne 85 and elabCode ne 86 and elabCode ne 87 and elabCode ne 88 then begin  ; week-days, summer...
    ...
    plot, [-Xaxis,Xaxis], color=0,/nodata, xtitle=ntxt1,ytitle='', title='DynamicEvaluation'+'   '+pars,$
      background=255,yrange=[-Yaxis,Yaxis],xrange=[-Xaxis,Xaxis],xstyle=1,ystyle=1, $
      position=plotter->getPosition(), noerase=plotter->getOverplotKeyword(0)
    ;KeesC 06FEB2015
    hx=-Xaxis-1.85*Xaxis/12.
    hy1= 2.*Yaxis/12. & hy2=1.*Yaxis/12.
    ; 2015 April MM
    ;setUserFont, 'UserDef3', FORCELOG=FORCELOG
    if plotter->currentDeviceIsPostscript() then setUserFont, 'PSFont', FORCELOG=FORCELOG else setUserFont, 'DiagramFont', FORCELOG=FORCELOG
    ;xyouts,hx,hy1,titleY1,charsize=2,charthick=1,color=0
    ;xyouts,hx,hy2,titleY2,charsize=2,charthick=1,color=0
    xyouts,hx,hy1,titleY1,color=0
    xyouts,hx,hy2,titleY2,color=0
    ...
    ; 2015 April MM
    ;setUserFont, 'UserDef4', FORCELOG=FORCELOG
    if plotter->currentDeviceIsPostscript() then setUserFont, 'PSFont', FORCELOG=FORCELOG else setUserFont, 'DiagramFont', FORCELOG=FORCELOG
    ...
    xyouts,-0.97*Xaxis,-0.1*Yaxis,'dMod<dObs',color=0
    xyouts,0.57*Xaxis,0.05*Yaxis,'dMod<dObs',color=0
    xyouts,-0.43*Xaxis,-0.95*Yaxis,'dMod>dObs',color=0
    xyouts,0.02*Xaxis,0.89*Yaxis,'dMod>dObs',color=0
  ...
  if elabCode eq 85 or elabCode eq 87 then begin ;potential calculations
    ...
    ReductionPlotOrig=strmid(scenarioCodes[1:n_elements(scenarioCodes)-1],3,2)
    ahlp=ReductionPlotOrig(sort(ReductionPlotOrig))
    ReductionPlot=ahlp(uniq(ahlp))
    if nobsg gt 0 and n_elements(ReductionPlot) gt 1 then begin
      a=dialogMsg(['Number of groups >= 1','Number of reductions >= 2'], /information, FORCELOG=FORCELOG)
    endif
    
    colorTest=0
    ytitle=pollutantPlot
    if where(strupcase(ytitle) eq 'ALL') gt -1 then ytitle=[pollutantPlot,'INT']
    mypsym,9,1
    fontFactor=3.
    ttThick=1.0
    ...
    for i=0,n_elements(ytitle)-1 do begin  ;overall display
      ...
      plots,[0.15,0.95],[yval-0.08,yval-0.08],/normal,color=colortest
      plots,[0.15,0.15],[yval,yval-0.08],/normal,color=colortest
      plots,[0.95,0.95],[yval,yval-0.08],/normal,color=colortest
      
      plots,[0.31,0.31],[yval,yval-0.08],/normal,color=colortest
      plots,[0.47,0.47],[yval,yval-0.08],/normal,color=colortest,linestyle=2
      plots,[0.63,0.63],[yval,yval-0.08],/normal,color=colortest,linestyle=2
      plots,[0.79,0.79],[yval,yval-0.08],/normal,color=colortest,linestyle=2
      
      ;2015 April MM, font properties are code derived
      ; just set vector
      ...
      xyouts,0.10,yval-0.075,'Epis',/normal,color=colortest,charsize=0.75*fontFactor, charthick=ttThick ;, FONT=1
      
    endfor
    nScenCodes=n_elements(scenarioCodes)
    if nobsG ge 1 then begin
      recognizeHighLight=bytarr(2*nobsG*(nScenCodes-1)+nobsG*4)   ; 8
      recognizeRegionEdges=ptrarr(2*nobsG*(nScenCodes-1)+nobsG*4)
      recognizeNames=strarr(2*nobsG*(nScenCodes-1)+nobsG*4)
      recognizeValues=strarr(2*nobsG*(nScenCodes-1)+nobsG*4)
      recognizePoint=fltarr(4,2)
      recognizeRangeX=0.005
      recognizeRangeY=0.005
    endif
    ijrec=-1
    for itype=0,3 do begin   ; big small big small
      for is=1,nScenCodes-1 do begin
        pollC=strmid(scenarioCodes(is),0,3)   ;All NH3
        reduC=strmid(scenarioCodes(is),3,2)   ; 15
        ccP=where(pollC eq PollutantPlot)   ; to fix yval
        ccR=where(reduC eq ReductionPlot)   ; ccR=0 15 (small)    ccR=1  40 (large)
        if ccR eq 0 and n_elements(ReductionPlot) ne 1 then isize=1.5
        if ccR eq 1 or n_elements(ReductionPlot) eq 1 then isize=2.5
        if ccR eq 0 then begin
          icol1=5
          icol2=4
          ithk=4
        endif
        if ccR eq 1 then begin
          icol1=3
          icol2=2
          ithk=10
        endif
        yval=0.90-ccP[0]*0.12
        if nobs gt 0 then begin
          if (itype eq 0 and CCR eq 1) or (itype eq 1 and CCR eq 0) then begin
            ; yearly averages
            plots,[0.31+0.64*min(allDataXY[0,0,is,*,0]),0.31+0.64*max(allDataXY[0,0,is,*,0])],[yval-0.02,yval-0.02],/normal,color=icol1,thick=ithk
            mypsym,9,1
            plots,[0.31+0.64*mean(allDataXY[0,0,is,*,0]),yval-0.02],psym=8,/normal,color=icol1,symsize=isize ;Reg
            ; episodes
            plots,[0.31+0.64*min(allDataXY[0,0,is,*,1]),0.31+0.64*max(allDataXY[0,0,is,*,1])],[yval-0.06,yval-0.06],/normal,color=icol2,thick=ithk
            mypsym,9,1
            plots,[0.31+0.64*mean(allDataXY[0,0,is,*,1]),yval-0.06],psym=8,/normal,color=icol2,symsize=isize ;Reg
          endif
        endif
        if nobsG ge 1 and itype ge 2 then begin
          for ig=0,nobsG-1 do begin
            ;year
            icol2=ig+2
            if nobsG eq 1 then begin
              icol2=3-ccR
            endif
            if (itype eq 2 and CCR eq 1) or (itype eq 3 and CCR eq 0) then begin
              mypsym,9,1
              plots,[0.31+0.64*allDataXY[0,0,is,nobs+ig,0],yval-0.02],psym=8,/normal,color=icol2,symsize=isize  ;rec
              ;              mypsym,13,1
              ;              plots,[0.31+0.64*allDataXY[0,0,is,nobs+ig,0],yval-0.02],psym=8,/normal,color=0,symsize=isize
              ;epis
              mypsym,4,1
              plots,[0.31+0.64*allDataXY[0,0,is,nobs+ig,1],yval-0.06],psym=8,/normal,color=icol2,symsize=isize   ;rec
              ;              mypsym,13,1
              ;              plots,[0.31+0.64*allDataXY[0,0,is,nobs+ig,1],yval-0.06],psym=8,/normal,color=0,symsize=isize
              
              for jrec=0,1 do begin
                ijrec=ijrec+1
                ;                print,'ijrec= ',ijrec
                x0=0.31+0.64*allDataXY[0,0,is,nobs+ig,jrec]
                y0=yval-0.02-jrec*0.04
                recognizePoint[0,*]=[x0-recognizeRangeX,y0-recognizeRangeY]
                recognizePoint[1,*]=[x0-recognizeRangeX,y0+recognizeRangeY]
                recognizePoint[2,*]=[x0+recognizeRangeX,y0+recognizeRangeY]
                recognizePoint[3,*]=[x0+recognizeRangeX,y0-recognizeRangeY]
                recognizePoint1=transpose(recognizePoint)
                normRecognizePoint=recognizePoint1  ;convert_coord(recognizePoint1, /DATA, /TO_NORMAL)
                normRecognizePoint=transpose(normRecognizePoint)
                normRecognizePoint=normRecognizePoint[*, 0:1]
                recognizePointPtr=ptr_new(normRecognizePoint, /NO_COPY)
                recognizeRegionEdges[ijrec]=recognizePointPtr
                recognizeNames[ijrec]=groupTitles(ig)+' '+reductionplot(ccr)+'%'
                x00=-(x0-.31)/(.95-.31)
                recognizeValues[ijrec]=strcompress(x00,/remove_all)
              endfor
            endif
          endfor
        endif
    ...
    ; interaction term
    if where(strupcase(ytitle) eq 'ALL') gt -1 then begin
    
      yval=0.90-n_elements(PollutantPlot)*0.12
      
      interaction=fltarr(nobs+nobsG,2,2)  ;nobs , reduction level, year-episodes
      interaction(*,*,*)=!values.f_nan
      for ir=0, n_elements(ReductionPlot)-1 do begin     ; 15 40
        ccALL=where(strupcase(PollutantPlotOrig) eq 'ALL' and ReductionPlot(ir) eq ReductionPlotOrig,countALL)
        ccCMP=where(strupcase(PollutantPlotOrig) ne 'ALL' and ReductionPlot(ir) eq ReductionPlotOrig,countCMP)
        ccALL=ccALL+1
        ccCMP=ccCMP+1
        if countALL ge 0 and countCMP ge 0 then begin   ;i,0,0,
          for i=0,nobs+nobsG-1 do interaction(i,ir,0)=allDataXY[0,0,ccALL,i,0]-total(allDataXY[0,0,ccCMP,i,0])
          for i=0,nobs+nobsG-1 do interaction(i,ir,1)=allDataXY[0,0,ccALL,i,1]-total(allDataXY[0,0,ccCMP,i,1])
        endif
      endfor
      if nobs gt 0 then begin
        ;year
        plots,[0.31+0.64*min(interaction[0:nobs-1,1,0]),0.31+0.64*max(interaction[0:nobs-1,1,0])],[yval-0.02,yval-0.02],/normal,color=3,thick=10
        mypsym,9,1
        plots,[0.31+0.64*mean(interaction[0:nobs-1,1,0]),yval-0.02],psym=8,/normal,color=3,symsize=2.5  ;rec
        
        plots,[0.31+0.64*min(interaction[0:nobs-1,0,0]),0.31+0.64*max(interaction[0:nobs-1,0,0])],[yval-0.02,yval-0.02],/normal,color=5,thick=4
        mypsym,9,1
        plots,[0.31+0.64*mean(interaction[0:nobs-1,0,0]),yval-0.02],psym=8,/normal,color=5,symsize=1.5  ;rec
        
        ;epis
        plots,[0.31+0.64*min(interaction[0:nobs-1,1,1]),0.31+0.64*max(interaction[0:nobs-1,1,0])],[yval-0.06,yval-0.06],/normal,color=2,thick=10
        mypsym,9,1
        plots,[0.31+0.64*mean(interaction[0:nobs-1,1,1]),yval-0.06],psym=8,/normal,color=2,symsize=2.5  ;rec
        
        plots,[0.31+0.64*min(interaction[0:nobs-1,0,1]),0.31+0.64*max(interaction[0:nobs-1,0,0])],[yval-0.06,yval-0.06],/normal,color=4,thick=4
        mypsym,9,1
        plots,[0.31+0.64*mean(interaction[0:nobs-1,0,1]),yval-0.06],psym=8,/normal,color=4,symsize=1.5  ;rec
        
      endif
      if nobsG gt 0 then begin
        for ig=0,nobsG-1 do begin
          ;year
          col1=ig+2
          col2=ig+2
          ...
          mypsym,9,1
          plots,[0.31+0.64*mean(interaction[nobs+ig,1,0]),yval-0.02],psym=8,/normal,color=col1,symsize=2.5   ;rec
          ;        mypsym,13,1
          ;        plots,[0.31+0.64*mean(interaction[nobs+ig,1,0]),yval-0.02],psym=8,/normal,color=0,symsize=2.
          ;        mypsym,9,1
          plots,[0.31+0.64*mean(interaction[nobs+ig,0,0]),yval-0.02],psym=8,/normal,color=col2,symsize=2.5   ;rec
          ;        mypsym,13,1
          ;        plots,[0.31+0.64*mean(interaction[nobs+ig,0,0]),yval-0.02],psym=8,/normal,color=0,symsize=1.5
          ;epis
          mypsym,4,1
          plots,[0.31+0.64*mean(interaction[nobs+ig,1,1]),yval-0.06],psym=8,/normal,color=col1,symsize=2.5   ;rec
          ;        mypsym,13,1
          ;        plots,[0.31+0.64*mean(interaction[nobs+ig,1,1]),yval-0.06],psym=8,/normal,color=0,symsize=2.
          ;        mypsym,9,1
          plots,[0.31+0.64*mean(interaction[nobs+ig,0,1]),yval-0.06],psym=8,/normal,color=col2,symsize=2.5   ;rec
          ;        mypsym,13,1
          ;        plots,[0.31+0.64*mean(interaction[nobs+ig,0,1]),yval-0.06],psym=8,/normal,color=0,symsize=1.5
          for jrec=0,1 do begin
            for irec=0,1 do begin
              ijrec=ijrec+1
              ;            print,'ijrec= ',ijrec
              x0=0.31+0.64*mean(interaction[nobs+ig,irec,jrec])
              y0=yval-0.02-jrec*0.04
              recognizePoint[0,*]=[x0-recognizeRangeX,y0-recognizeRangeY]
              recognizePoint[1,*]=[x0-recognizeRangeX,y0+recognizeRangeY]
              recognizePoint[2,*]=[x0+recognizeRangeX,y0+recognizeRangeY]
              ...
              recognizePointPtr=ptr_new(normRecognizePoint, /NO_COPY)
              recognizeRegionEdges[ijrec]=recognizePointPtr
              if n_elements(ReductionPlot) eq 1 then recognizeNames[ijrec]=groupTitles(ig)+' '+reductionplot+'%'
              if n_elements(ReductionPlot) gt 1 then recognizeNames[ijrec]=groupTitles(ig)+' '+reductionplot(irec)+'%'
              x00=-(x0-.31)/(.95-.31)
              recognizeValues[ijrec]=strcompress(x00,/remove_all)
            endfor
          endfor
      ...
      if nobsG gt 0 then rInfo = obj_new("RecognizeInfo", recognizeNames, recognizeValues, recognizeHighLight, recognizeRegionEdges)
      if nobsG gt 0 then plotInfo->setRecognizeInfo, rInfo
    endif
    
  endif  ; end elaboration 85
    ...
    plot, indgen(1),color=255,/nodata,xrange=[0,10],xstyle=1,position=plotter->getPosition(), noerase=plotter->getOverplotKeyword(0)
    ...
    for i=0,n_elements(ytitle)-1 do begin  ;overall display
      ...
      xyouts,0.61,yval-0.11,xticks(3),/normal,color=colortest,charsize=0.7*fontFactor, charthick=ttThick ;, FONT=1
      xyouts,0.77,yval-0.11,xticks(4),/normal,color=colortest,charsize=0.7*fontFactor, charthick=ttThick ;, FONT=1
      xyouts,0.95,yval-0.11,xticks(5),/normal,color=colortest,charsize=0.7*fontFactor, charthick=ttThick ;, FONT=1
  ...
  if count1 ge 1 then color1(cc1)=100
    ...
    cc1=where(plotValues ge 0.*varmax and plotValues le 0.2*varmax,count1)
  if count1 ge 1 then color1(cc1)=40
  ...
  for iobs=0,nobs-1 do begin
    ...
    recognizeRegionEdges[iObs]=recognizePointPtr
      ;KeesC 03NOV2015
      recognizeNames[iObs]=legNames[iObs,3]
...
PRO FM_PLOTDYNAMICEVALUATIONLEGEND, plotter, request, result
  ...
  elabcode=request->getElaborationCode()
  ; KeesC 03NOV2015 ?
  if elabcode eq 88 then begin
    legendGenericBuild88,request,result,plotter    ;88
  endif else begin
  if elabCode ne 85 and elabCode ne 86 and elabCode ne 87 then begin
      legendGenericBuild,request,result,plotter    ;88
  endif else begin
...
PRO FM_PlotCategory, plotter, request, result
  ...
  xyouts,110,n_elements(legnames)*.9,strtrim(fix(100.*float(countgood)/countall),2)+'%',/data,color=0
  rInfo = obj_new("RecognizeInfo", recognizeNames, recognizeValues, recognizeHighLight, recognizeRegionEdges)
  plotInfo->setRecognizeInfo, rInfo
...
PRO FM_PlotTimeSeries, plotter, request, result, allDataXY, allDataColor, allDataSymbol
  ...
  ;setUserFont, 'UserDef7', FORCELOG=FORCELOG
  if plotter->currentDeviceIsPostscript() then setUserFont, 'PSFont', FORCELOG=FORCELOG else setUserFont, 'diagramFont', FORCELOG=FORCELOG
  ...
  ;KeesC 17JAN2013
  ;!p.font=-1
  ;etDeviceFont, fontName='System', /STANDARD
  ;device,set_font=getBestFont(fontName='System', /STANDARD), /TT_FONT
  setUserFont, /RESET
END
...
PRO FM_PlotTimeSeriesLegend, plotter, request, result
  ...
  for i=0, symbolSequenceNo-1 do begin
    ...
    legendCoordsXYouts=[thisStartX+legoWidth+.01, startY-.01]
    ...
    ;2015 April MM
    ;lastUserFont=getUserFont()
    ;setUserFont, 'UserDef8', FORCELOG=FORCELOG
    ;setUserFont, 'UserDef18', FORCELOG=FORCELOG
    if plotter->currentDeviceIsPostscript() then setUserFont, 'PSFont', FORCELOG=FORCELOG else setUserFont, 'LegendFont', FORCELOG=FORCELOG
    ...
    xyouts, legendCoordsXYouts[0], legendCoordsXYouts[1], strmid(legNames[i],0,7), COLOR=0, /NORM,  WIDTH=textWidth
  ;setUserFont, lastUserFont
  endfor
...
PRO FM_PlotScatter, plotter, request, result
  ...
  multipleTitle=''
  !y.range=0
  ; set a valid multiple draw title MM+PT May 2015
  ; MM+PT May 2015
  fName=request->getFilename()
  multipleTitle=''
  if fName ne '' then begin
    fsm=obj_new('FMFileSystemManager')
    multipleTitle=fsm->getBaseFileName(fName)
    idx=strpos(multipleTitle, '_')
    if idx gt 0 then multipleTitle=strmid(multipleTitle,0, idx)
    obj_destroy, fsm
  endif
  plotter->wsetMainDataDraw
  silentMode=plotter->getSilentMode()
  ...
  ;  if infoSize.n_dimensions eq 2 then begin
  ;    silentMode=plotter->getSilentMode()
  ;    FORCELOG=silentMode
  ...
  if elabCode eq 50 then begin ; this is Mod1 vs Mod2
    ;KeesC 10FEB2015
  ...
  if elabCode eq 57 then begin
    ;KeesC 10FEB2015
    if n_elements(parCodes) ne 2 then begin
      a=dialogMsg('Not enough models for this elaboration', FORCELOG=FORCELOG)
      return
    endif
  ...
  if size_alldataXY(0) eq 2 then begin    ; scatter mean
      ...
      if criteria[0] gt 0 and isGroupSelection eq 0 then begin
        ...
        if countValidStations gt 0 then begin
          ...
          if strupcase(frequency) eq 'HOUR' then begin
            ...
            ;2015 April MM
            ;lastUserFont=getUserFont()
            ;setUserFont, 'UserDef10', FORCELOG=FORCELOG
            if plotter->currentDeviceIsPostscript() then setUserFont, 'PSFont', FORCELOG=FORCELOG else setUserFont, 'DiagramFont', FORCELOG=FORCELOG
            ...
            xyouts,0.16,0.87,'valid/selected stations/groups: '+$
              strtrim(validStationNb,2)+'/'+strtrim(totalStationNb,2),$
              color=0,/normal,charsize=1.5*psFact
          ;setUserFont, lastUserFont
          endif
        endif
      endif
    ...
    for iobs=0, nObs_param-1 do begin
      ...
      ;2015 April MM
      ;lastUserFont=getUserFont()
      ;setUserFont, 'UserDef11', FORCELOG=FORCELOG
      if plotter->currentDeviceIsPostscript() then setUserFont, 'PSFont', FORCELOG=FORCELOG else setUserFont, 'LegendFont', FORCELOG=FORCELOG
      ;xyouts,0.80,0.30-iobs*0.05,'R2 = '+strtrim(R2,2),/normal,color=2+allDataColor(iobs),charthick=2,charsize=1.3
      xyouts,0.80,0.30-iobs*0.05,'R2 = '+strtrim(R2,2),/normal,color=2+allDataColor(iobs)
    ;setUserFont, lastUserFont
    endfor
  endelse
  ;KeesC 07FEB2015
...
PRO FM_PlotGeoMapLegend, plotter, request, result
  ...
  if strupcase(frequency) eq 'HOUR' then begin
    ...
    xyouts, 0.07,0.43, 'R dominated',COLOR=0,/NORMal
    ...
    xyouts, 0.07,0.28, 'Sigma dominated',COLOR=0,/NORMal
  ;setUserFont, lastUserFont
  endif
  if strupcase(frequency) eq 'YEAR' then begin
    mypsym,9,2
    ...
    xyouts, 0.07,0.73, 'Criterium > 1',COLOR=0,/NORMal
  ;setUserFont, lastUserFont
  endif
  legendInfo,request,result,plotter
  ;!p.font=-1
...
PRO FM_PlotGoogleEarth, plotter, request, result
  ...
  if keyword_set(silentMode) then begin
    rsult=dialogMsg(['GoogleEarth not available for current mode'], FORCELOG=FORCELOG)
    return
  endif
  plotter->wsetMainDataDraw
  tpInfo=result->getGenericPlotInfo()
  ...
  if error_status NE 0 THEN BEGIN
    ...
    errMsg=dialogMsg([['Please check path to Google Earth location'], fullString, [' Check existence or read permission'], [' and modify -HOME/resource/init.ini- accordingly.']], /ERROR, FORCELOG=FORCELOG)
    return
  endif
...
PRO FM_PlotTaylor, plotter, request, result
  ...
  for i=0,8 do begin
    ...
    xyouts,xtxt,ytxt,strmid(strtrim(string(cc),2),0,4),color=0
  ;setUserFont, lastUserFont
  endfor
  ...
  ;    alignment=.5,color=0
  xyouts,.85*max_s,0.75*max_s,'Corr_Coef',orientation=-35.,$
    alignment=.5,color=0
  ;setUserFont, lastUserFont
  ...
  for k=1,5 do begin
    ...
    if jlab eq 360 then begin
      ...
      ;        color=0
      xyouts,sd_obs+i*xhlp(360),.02*max_s,strmid(strtrim(string(i),2),0,3), $
        color=0
    endif else begin
      ;      xyouts,.02*max_s,.01*max_s+i*thetahlp(jlab),strmid(strtrim(string(i),2),0,3),charsize=1.,$
      ;        color=0
      xyouts,.02*max_s,.01*max_s+i*thetahlp(jlab),strmid(strtrim(string(i),2),0,3),$
        color=0
    endelse
  ;setUserFont, lastUserFont
  endfor
...
PRO FM_PlotTarget, plotter, request, result, allDataXY, allDataColor, allDataSymbol
  ...
  if (criteria eq 0 or countValidStations eq 0) then begin
    ...
    xyouts,1,9,'No Diagram available for current choice',/data,color=0
    ;setUserFont, 'UserDef7', FORCELOG=FORCELOG
    ...
    xyouts,2,7,'Check criteria availability for selected parameter',/data,color=0
    xyouts,2,6,'Only available for 8hmax O3 daily PM10 and hourly NO2',/data,color=0
    xyouts,2,5,'If group selected only worst statistics works',/data,color=0
    xyouts,2,4,'Might be no valid stations or groups selected',/data,color=0
    xyouts,2,3,'For annual MQO entire year should be selected',/data,color=0
    ;setUserFont, lastUserFont
    goto,jumpend
  endif
  ...
  ;2015 April MM
  ;lastUserFont=getUserFont()
  ;setUserFont, 'UserDef13', FORCELOG=FORCELOG
  if plotter->currentDeviceIsPostscript() then setUserFont, 'PSFont', FORCELOG=FORCELOG else setUserFont, 'DiagramFont', FORCELOG=FORCELOG
  ...
  if elabcode eq 74 then begin  ;forecast
    ...
    xyouts, plotRange*0.65, -plotRange*0.07,  'FA > MA', color=0,/data,charsize=facSize*1.5
  endif else begin
    ...
    xyouts, -plotRange*0.95, -plotRange*0.07, 'R', color=0,/data,charsize=facSize*1.5
...
PRO FM_PlotTargetHDY, plotter, request, result, allDataXY, allDataColor, allDataSymbol
  multipleTitle=''
  !y.range=0
  ; set a valid multiple draw title MM+PT May 2015
  ; MM+PT May 2015
...
PRO FM_PlotSoccer, plotter, request, result, allDataXY, allDataColor, allDataSymbol
  ...
  if checkDataNan(allDataXY) then begin
    ...
    ;2015 April MM
    ;lastUserFont=getUserFont()
    ;setUserFont, 'UserDef6', FORCELOG=FORCELOG
    ;xyouts,1,5,'No valid stations or groups selected',charsize=2,charthick=2,/data,color=0
    xyouts,1,5,'No valid stations or groups selected',/data,color=0
    ;setUserFont, lastUserFont
    goto,jumpend
  ...
  if elabcode ne 20 and elabCode ne 22 and elabCode ne 34 then begin
    ...
    plot, indgen(10), color=0,xrange=[-maxxAxis,maxxAxis], yrange=[0,maxyAxis], xstyle=1,ystyle=1,/nodata, $
      xtitle='BIAS',ytitle='RMSE',title='Soccer PLOT'+'   '+pars, background=255, position=plotter->getPosition(), noerase=plotter->getOverplotKeyword(0)
    ;setUserFont, lastUserFont
    ...
    plot, indgen(10), color=0,xrange=[-maxxAxis,maxxAxis], yrange=[-maxyAxis,maxyAxis], xstyle=1,ystyle=1,/nodata, $
      xtitle=' ',ytitle=' ',title='Soccer AQ PLOT'+'   '+pars, background=255, position=plotter->getPosition(), noerase=plotter->getOverplotKeyword(0)
    ;setUserFont, lastUserFont
    ...
    ;2015 April MM
    ;lastUserFont=getUserFont()
    ;setUserFont, 'UserDef14', FORCELOG=FORCELOG
    if plotter->currentDeviceIsPostscript() then setUserFont, 'PSFont', FORCELOG=FORCELOG else setUserFont, 'LegendFont', FORCELOG=FORCELOG
    ...
    xyouts,0.2,2.7,'Bias+',/data,color=0
    xyouts,1.5,2.7,'Bias+ / R',/data,color=0
    xyouts,1.8,0.1,'R',/data,color=0
    xyouts,1.5,-2.7,'Bias- / R',/data,color=0
    xyouts,0.2,-2.7,'Bias-',/data,color=0
    xyouts,-0.6,0.1,'SD',/data,color=0
    xyouts,-0.9,2.7,'SD / Bias+',/data,color=0
    xyouts,-0.9,-2.7,'SD / Bias-',/data,color=0
    xyouts,-2.9,2.7,'SD / Bias+ / R',/data,color=0
    xyouts,-2.9,-2.7,'SD / Bias- / R',/data,color=0
    xyouts,-2.9,0.1,'SD / R',/data,color=0
    ;setUserFont, lastUserFont
...
PRO FM_PlotTable2, plotter, request, result
  ...
  if criteria eq 0 then begin
    ...
    ;    xyouts,2,3,'For annual MQO entire year should be selected',charsize=1.5,charthick=1,/data,color=0
    xyouts,2,7,'Check criteria availability for selected parameter',/data,color=0
    xyouts,2,6,'Only available for 8hmax O3 daily/yearly PM10 and hourly/yearly NO2',/data,color=0
    xyouts,2,5,'If group selected only worst statistics works',/data,color=0
    xyouts,2,4,'Might be no valid stations or groups selected',/data,color=0
    xyouts,2,3,'For annual MQO entire year should be selected',/data,color=0
    ;setUserFont, lastUserFont
  ...
  if strupcase(frequency) eq 'HOUR' then begin  ;Hourly/daily values
    ...
    ;Result->setMultipleDrawMainTitle, 'SUMMARY STATISTICS'
    xyouts,xmin+0.15,ymax-0.05,'SUMMARY STATISTICS',charsize=1.5*facsize*psFact,/data,color=0
    ;setUserFont, lastUserFont
    ;charsize & charthick are both code derived!
    ...
    ;setUserFont, 'UserDef15', FORCELOG=FORCELOG
    if plotter->currentDeviceIsPostscript() then setUserFont, 'PSFont', FORCELOG=FORCELOG else setUserFont, 'diagramFont', FORCELOG=FORCELOG
    ...
    for ii=0,7 do begin
      ...
      if ii ge 2 then begin
        ...
        xyouts,xmin+0.005,ymax-0.18-ii*0.095,res1(0),/data,charsize=1.2*facSize*psFact,color=3
        xyouts,xmin+0.035,ymax-0.22-ii*0.095,res1(1),/data,charsize=1.0*facSize*psFact,color=3
      endif else begin
        ;KeesC 18SEP2014
        ; ii=1
        ...
        xyouts,xmin+0.005,yii,legnames(ii),/data,charsize=1.2*facSize*psFact,color=3
        if ii eq 1 then begin
          ;setUserFont, 'UserDef12', FORCELOG=FORCELOG
          if plotter->currentDeviceIsPostscript() then setUserFont, 'PSFont', FORCELOG=FORCELOG else setUserFont, 'LegendFont', FORCELOG=FORCELOG
          ;xyouts,xmin+0.005,yii-0.035,ExcVal+' '+mus[0],/data,charsize=facSize*psFact,color=0,charthick=1.5
          xyouts,xmin+0.005,yii-0.035,ExcVal+' '+mus[0],/data,charsize=facSize*psFact,color=0
        endif
      endelse
      ;setUserFont, 'UserDef15', FORCELOG=FORCELOG
      if plotter->currentDeviceIsPostscript() then setUserFont, 'PSFont', FORCELOG=FORCELOG else setUserFont, 'LegendFont', FORCELOG=FORCELOG
      ...
      xyouts,xmin+0.25,ymax-0.23-ii*0.095,strtrim(minVal(ii),2),/data,charsize=0.9*facSize*psFact,alignment=0.5,color=0
      xyouts,xmax-0.10,ymax-0.23-ii*0.095,strtrim(maxVal(ii),2),/data,charsize=0.9*facSize*psFact,alignment=0.5,color=0
      ;setUserFont, lastUserFont
      ...
      for ix=0,fillint(ii)-1 do begin
        ...
        xyouts,xmin+0.25+(xmax-0.10-xmin-0.25)*xx(ix),ymax-0.23-ii*0.095,$
          fillstr(ii,ix),/data,charsize=0.9*facSize*psFact,alignment=0.5,color=0
        plots,[xmin+0.25+(xmax-0.10-xmin-0.25)*xx(ix),xmin+0.25+(xmax-0.10-xmin-0.25)*xx(ix)],$
          [ymax-0.20-ii*0.095,ymax-0.19-ii*0.095],color=0,/data,thick=2,linestyle=0
      endfor
      ;setUserFont, lastUserName
    ...
    xyouts,xmin-0.035,ymax-0.205,'O',/data,charsize=1.5*psFact,color=3
    xyouts,xmin-0.035,ymax-0.24,'B',/data,charsize=1.5*facsize*psFact,color=3
    xyouts,xmin-0.035,ymax-0.275,'S',/data,charsize=1.5*facsize*psFact,color=3
    
    xyouts,xmin-0.037,ymax-0.495,'T',/data,charsize=1.5*facsize*psFact,color=3
    xyouts,xmin-0.03,ymax-0.53,'I',/data,charsize=1.5*facsize*psFact,color=3
    xyouts,xmin-0.035,ymax-0.565,'M',/data,charsize=1.5*facsize*psFact,color=3
    xyouts,xmin-0.035,ymax-0.60,'E',/data,charsize=1.5*facsize*psFact,color=3
    ;  ;
    xyouts,xmin-0.035,ymax-0.765,'S',/data,charsize=1.5*facsize*psFact,color=3
    xyouts,xmin-0.035,ymax-0.80,'P',/data,charsize=1.5*facsize*psFact,color=3
    xyouts,xmin-0.035,ymax-0.835,'A',/data,charsize=1.5*facsize*psFact,color=3
    xyouts,xmin-0.035,ymax-0.87,'C',/data,charsize=1.5*facsize*psFact,color=3
    xyouts,xmin-0.035,ymax-0.905,'E',/data,charsize=1.5*facsize*psFact,color=3
  ;setUserFont, lastUserFont
  ...
  ;******************YEARLY*********************************************
    ...
    xyouts,xmin+0.10,ymax-0.05,'SUMMARY Yearly STATISTICS',charsize=1.5*facsize*psFact,/data,color=0
    ;setUserFont, lastUserFont
    xyouts,xmin+0.50,ymax-0.05,'    Nb of stations/groups: '+strtrim(allDataAxis(1),2)+' valid / '+strtrim(allDataAxis(0),2)+' selected',/data,charsize=1.0*facsize*psFact,color=0
    ;setUserFont, 'UserDef15', FORCELOG=FORCELOG
    if plotter->currentDeviceIsPostscript() then setUserFont, 'PSFont', FORCELOG=FORCELOG else setUserFont, 'diagramFont', FORCELOG=FORCELOG
    ;xyouts,xmin+0.01,ymax-0.12,'INDICATOR',/data,charsize=1.2*facsize*psFact,color=0,charthick=2.3
    xyouts,xmin+0.01,ymax-0.12,'INDICATOR',/data,charsize=1.2*facsize*psFact,color=0
    ;setUserFont, lastUserFont
    ...
    for ii=0,3 do begin
      ...
      if ii ge 1 then begin
        ...
        xyouts,xmin+0.005,ymax-0.23-ii*deltaY,res1(0),/data,charsize=1.2*facsize*psFact,color=3
        xyouts,xmin+0.035,ymax-0.27-ii*deltaY,res1(1),/data,charsize=1.0*facsize*psFact,color=3
      endif else begin
        ;        xyouts,xmin+0.005,ymax-0.25-ii*deltaY,legnames(ii),/data,charsize=1.2*facsize*psFact,color=3,charthick=2.3
        xyouts,xmin+0.005,ymax-0.25-ii*deltaY,legnames(ii),/data,charsize=1.2*facsize*psFact,color=3
      endelse
      ...
      xyouts,xmin+0.25,ymax-0.285-ii*deltaY,strtrim(minVal(ii),2),/data,charsize=0.9*facsize*psFact,alignment=0.5,color=0
      xyouts,xmax-0.10,ymax-0.285-ii*deltaY,strtrim(maxVal(ii),2),/data,charsize=0.9*facsize*psFact,alignment=0.5,color=0
      ;KeesC 24NOV2013
      if ii eq 0 then xx=[0.2,0.4,0.6,0.8]
    ...
    xyouts,xmin-0.035,ymax-0.205,'O',/data,charsize=1.5*facsize*psFact,color=3
    xyouts,xmin-0.035,ymax-0.24,'B',/data,charsize=1.5*facsize*psFact,color=3
    xyouts,xmin-0.035,ymax-0.275,'S',/data,charsize=1.5*facsize*psFact,color=3
    
    xyouts,xmin-0.037,ymax-0.415,'T',/data,charsize=1.5*facsize*psFact,color=3
    xyouts,xmin-0.03,ymax-0.45,'I',/data,charsize=1.5*facsize*psFact,color=3
    xyouts,xmin-0.035,ymax-0.485,'M',/data,charsize=1.5*facsize*psFact,color=3
    xyouts,xmin-0.035,ymax-0.52,'E',/data,charsize=1.5*facsize*psFact,color=3
    ;  ;
    xyouts,xmin-0.035,ymax-0.66,'S',/data,charsize=1.5*facsize*psFact,color=3
    xyouts,xmin-0.035,ymax-0.695,'P',/data,charsize=1.5*facsize*psFact,color=3
    xyouts,xmin-0.035,ymax-0.73,'A',/data,charsize=1.5*facsize*psFact,color=3
    xyouts,xmin-0.035,ymax-0.765,'C',/data,charsize=1.5*facsize*psFact,color=3
    xyouts,xmin-0.035,ymax-0.80,'E',/data,charsize=1.5*facsize*psFact,color=3
  ;  ;
...
PRO FM_PlotTargetLegend, plotter, request, result
  ...
  jumpend:
END
PRO FM_PlotTargetHDYLegend, plotter, request, result
...
PRO FM_PlotBugle, plotter, request, result, allDataXY, allDataColor, allDataSymbol
  ...
  if elabcode eq 25 or elabCode eq 79 or elabCode eq 32 or elabcode eq 94 then begin  ;NMSD
    ...
    plot, indgen(fix(maxxaxis+1)), color=0,/nodata, xtitle='RMSU/SigO'+'   '+pars,ytitle='NMSD', title='MPC PLOT', background=255,$
      yrange=[-ymax,ymax],xrange=[minxAxis,maxxAxis],xstyle=1,ystyle=1, position=plotter->getPosition(), noerase=plotter->getOverplotKeyword(0)
    if criteria gt 0 then begin
      ; KeesC 1OCT2013
  ...
  if elabcode eq 15 or elabCode eq 78 or elabCode eq 18 or elabcode eq 93 then begin  ;R
    ...
    plot, indgen(fix(maxxaxis+1)), color=0,/nodata, xtitle='RMSU/SigO'+'   '+pars,ytitle='Correlation', title='MPC PLOT', background=255,$
      yrange=[ymin,ymax],xrange=[minxAxis,maxxAxis],xstyle=1,ystyle=1, position=plotter->getPosition(), noerase=plotter->getOverplotKeyword(0)
    if criteria gt 0 then begin
...
PRO FM_PlotQQ, plotter, request, result
  ...
  ;    yrange=[minAxis,maxAxis],xstyle=1,ystyle=1, position=plotter->getPosition(), noerase=plotter->getOverplotKeyword(0)
  plot, indgen(fix(maxaxis)), color=0,/nodata, xtitle='OBSERVED',ytitle='MODELLED', $
    title='QQ PLOT'+'   '+pars, background=255,xrange=[minAxis,maxAxis],$
    yrange=[minAxis,maxAxis],xstyle=1,ystyle=1, position=plotter->getPosition(), noerase=plotter->getOverplotKeyword(0)
...
PRO FM_PlotQQLegend, plotter, request, result
  ...
  if n_elements(modelCodes) gt 1 then begin ; many models
    ...
    for i=0, symbolSequenceNo-1 do begin
      ...
      xyouts, coords[0], coords[1], strmid(legNames[i],0,7), COLOR=0, /NORM,  WIDTH=textWidth
    endfor
  ;setUserFont, lastUserFont
...
PRO FM_PlotTable2legend, plotter, request, result
  ...
  ;2015 April MM
  ;lastUserFont=getUserFont()
  ;setUserFont, 'UserDef16', FORCELOG=FORCELOG
  if plotter->currentDeviceIsPostscript() then setUserFont, 'PSFont', FORCELOG=FORCELOG else setUserFont, 'LegendFont', FORCELOG=FORCELOG
  ;xyouts,x00, coords[1]-0.05,text1,/normal,color=0,charsize=1.25,charthick=1.25
  xyouts,x00, coords[1]-0.05,text1,/normal,color=0
  mypsym,5,2
  ...
  ;setUserFont, 'UserDef17', FORCELOG=FORCELOG
  if plotter->currentDeviceIsPostscript() then setUserFont, 'PSFont', FORCELOG=FORCELOG else setUserFont, 'LegendFont', FORCELOG=FORCELOG
  ;xyouts,x00, coords[1]-0.05,text2,/normal,color=0,charsize=1.25,charthick=2
  xyouts,x00, coords[1]-0.05,text2,/normal,color=0
  mypsym,9,2
  ...
  xyouts,x00, coords[1]-0.05,text3a,/normal,color=0
  ...
  xyouts,x00, coords[1]-0.2,text3b,/normal,color=0
  mypsym,9,2
  ...
  xyouts,x00, coords[1]-0.05,text4a,/normal,color=0
  ...
  xyouts,x00, coords[1]-0.2,text4b,/normal,color=0
  ;setUserFont, latFontUser
  jumpEnd:
...
PRO LEGENDINFO,request,result,plotter
  ...
  if resPoscript eq 0 then begin
    ;!p.font=0
    ;setDeviceFont, fontName='Arial', fontSize='12', fontType='bold', /FINE
    ;setUserFont, 'SmallFont', FORCELOG=FORCELOG
    if plotter->currentDeviceIsPostscript() then setUserFont, 'PSFont', FORCELOG=FORCELOG else setUserFont, 'LegendFont', FORCELOG=FORCELOG
  ;device,set_font=getBestFont(fontName='Arial', fontSize='12', fontType='bold', /FINE), /TT_FONT
  endif
  ...
  if extraValNumber eq 4 then extraValPrint=strmid(strtrim(extraVal(0),2),0,3)+'/'+$
    strmid(strtrim(extraVal(1),2),0,3)+'/'+$
    strmid(strtrim(extraVal(2),2),0,3)+'/'+$
    strmid(strtrim(extraVal(3),2),0,3)
  if extraValNumber eq 5 then extraValPrint=strmid(strtrim(extraVal(0),2),0,3)+'/'+$
    strmid(strtrim(extraVal(1),2),0,3)+'/'+$
    strmid(strtrim(extraVal(2),2),0,3)+'/'+$
    strmid(strtrim(extraVal(3),2),0,3)+'/'+$
    strmid(strtrim(extraVal(4),2),0,3)
  cooy=cooy-0.11
  coords=[0.77,cooy]
...
;*************
pro CheckCriteria, request, result, statistics, criteria, obsTimeSeries,alpha,criteriaOrig,LV,overwritefrequency=overwritefrequency
  startIndex=request->getStartIndex()
  endIndex=request->getEndIndex()
  parCodes=request->getParameterCodes()
  modelInfo=request->getModelInfo(overwritefrequency=overwritefrequency)
  year=modelInfo.year
  if 4*(fix(year)/4) ne fix(year) then begin  ; normal year
    Feb29start=59*24
    Feb29end=Feb29start+23
...
pro legendGenericBuild,request,result,plotter
  ...
  for i=0,i1 do begin
    ...
    xyouts, coords[0], coords[1] , strmid(legNamesPrint[i],0,13), COLOR=0, /NORM,  WIDTH=textWidth
  endfor
  ;setUserFont, lastUserFont
...
;**************
pro legendGenericBuild85,request,result,plotter
  ...
  if nobsS ge 1 then begin
    ...
    dy=.12
    dy2=.025
    plots,[startX,startX+.05],[startY,startY],/normal,color=3,thick=5
    xyouts,startX+.07,startY-dy2,'Range high red',/normal,color=0
    mypsym,9,1
    starty=starty-dy
    plots,startX+.01,startY,psym=8,/normal,color=3,symsize=2.
    mypsym,13,1
    plots,startX+.01,startY,psym=8,/normal,color=0,symsize=2.
    xyouts,startX+.07,startY-dy2,'Mean high red',/normal,color=0
    starty=starty-dy
    plots,[startX,startX+.05],[startY,startY],/normal,color=5,thick=5
    xyouts,startX+.07,startY-dy2,'Range low red',/normal,color=0
    mypsym,9,1
    starty=starty-dy
    plots,startX+.01,startY,psym=8,/normal,color=5,symsize=1.5
    mypsym,13,1
    plots,startX+.01,startY,psym=8,/normal,color=0,symsize=1.5
    xyouts,startX+.07,startY-dy2,'Mean low red',/normal,color=0
    starty=starty-dy
    plots,[startX,startX+.05],[startY,startY],/normal,color=2,thick=5
    xyouts,startX+.07,startY-dy2,'Range high red',/normal,color=0
    mypsym,9,1
    starty=starty-dy
    plots,startX+.01,startY,psym=8,/normal,color=2,symsize=2.
    mypsym,13,1
    plots,startX+.01,startY,psym=8,/normal,color=0,symsize=2.
    xyouts,startX+.07,startY-dy2,'Mean high red',/normal,color=0
    starty=starty-dy
    plots,[startX,startX+.05],[startY,startY],/normal,color=4,thick=5
    xyouts,startX+.07,startY,'Range low red',/normal,color=0
    mypsym,9,1
    starty=starty-dy
    plots,startX+.01,startY,psym=8,/normal,color=4,symsize=1.5
    mypsym,13,1
    plots,startX+.01,startY,psym=8,/normal,color=0,symsize=1.5
    xyouts,startX+.07,startY-dy2,'Mean low red',/normal,color=0
  endif
  if nobsG ge 1 then begin
    ...
    for ig=0,nobsG-1 do begin
      ...
      mypsym,9,1
      starty=starty-dy
      plots,startX+.01,startY,psym=8,/normal,color=col2,symsize=2.5
      ;      mypsym,13,1
      ;      plots,startX+.01,startY,psym=8,/normal,color=0,symsize=1.5
      ;      xyouts,startX+.07,startY-dy2,gtstr(ig)+': Mean 15%red',/normal,color=0
      xyouts,startX+.07,startY-dy2,gtstr(ig),/normal,color=0
    ;      mypsym,9,1
    ;      starty=starty-dy
    ;      plots,startX+.01,startY,psym=8,/normal,color=col1,symsize=2.
    ;;      mypsym,13,1
    ;;      plots,startX+.01,startY,psym=8,/normal,color=0,symsize=2.
    ;      xyouts,startX+.07,startY-dy2,gtstr(ig),/normal,color=0
    endfor
...
;*****************************************
pro legendGenericBuild88,request,result,plotter
  plotter->wsetInfoDataDraw
  silentMode=plotter->getSilentMode()
  FORCELOG=silentMode
  resPoscript=plotter->currentDeviceIsPostscript()
  ...
  plotValues=reform(allDataXy[0,0,1,*,0])
  maxval=max(plotvalues)
  startx =.03
  startY=0.9
  dy=.14
  dy2=.025
  color1=[0,40,100,150,210,250]
  mypsym,9,2
  plots,startX+.09,startY,psym=8,/normal,color=color1(0),symsize=1.2
  xyouts,startX+.12,startY-dy2,' <= 0. ',/normal,color=0,charsize=2.
  for i=1,5 do begin
    starty=starty-dy
    val0=strtrim(i*0.2*maxval,2)
    val1=strtrim((i+1.)*0.2*maxval,2)
    val00=strmid(val0,0,5)
    val11=strmid(val1,0,5)
    xyouts,startX,startY-dy2,val00+' < ',/normal,color=0,charsize=2.
    mypsym,9,2
    plots,startX+.09,startY,psym=8,/normal,color=color1(i),symsize=1.2
    xyouts,startX+.12,startY-dy2,'<= '+val11,/normal,color=0,charsize=2.
  endfor
  noplot:
end
;*****************************************
...
pro legendGenericBuildDiag0,request,result,plotter
  ...
  if diagramCode eq 0 then begin  ;barplot
    ...
    for i=0, n_elements(legprint)-1 do begin
      ...
      ;coords=[thisStartX+legoWidth+.01, startY+.002]
      coords=[thisStartX+legoWidth+.01, startY-.03]
      coords=plotter->legendNormalize(coords)
      ;      xyouts, coords[0], coords[1] , legPrint[i], COLOR=0, /NORM, charsize=.8, charthick=.8,  WIDTH=textWidth
      xyouts, coords[0], coords[1] , legPrint[i], COLOR=0, /NORM,  WIDTH=textWidth
    endfor
    ...
    if legprint2[0] ne '' then begin
      for i=0, n_elements(legprint2)-1 do begin
        ...
        startY=1.-((jheight+1)*legoHeight*2)
        thisStartX=startX+maxWidth+legoWidth+.02
        lego=[[thisStartX,startY], [thisStartX,startY+3*legoHeight], [thisStartX+3*legoWidth,startY+3*legoHeight], [thisStartX+3*legoWidth,startY], [thisStartX,startY]]
        coords=[thisStartX+(legoWidth/2), startY]
        coords=plotter->legendNormalize(coords)
        mypsym,5,1.5
        ...
        xyouts, coords[0], coords[1] , legPrint2[i], COLOR=0, /NORM,  WIDTH=textWidth
      endfor
    ;setUserFont, lastUserFont
    endif
  endif
...
pro mybar_plot,values,nsubbars,request,result, plotter,tickv,baselines=baselines,colors=colors,barnames=barnamesIn, $
