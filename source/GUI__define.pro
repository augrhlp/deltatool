FUNCTION GUI::getLabelPixWidth, suggested, row=row

 os=strupcase(!version.os_family)
 if os eq 'WIN' then value=suggested else value=suggested*1.5
 return, value

END

FUNCTION GUI::getLabelPixHeight, suggested, row=row
...
FUNCTION GUI::dialogMessage, textMessage, title=title, INFORMATION=INFORMATION, error=error, question=question, WARNING=WARNING, CENTER=CENTER

 return, self.mgr->dialogMessage(textMessage, dialog_parent=self.topBase, title=title, INFORMATION=INFORMATION, ERROR=ERROR, QUESTION=QUESTION, CENTER=CENTER)
...
;FUNCTION GUI::dialogMessage, textMessage, title=title, INFORMATION=INFORMATION, error=error, question=question, WARNING=WARNING, CENTER=CENTER
;
; return, dialog_message(textMessage, dialog_parent=self.topBase, title=title, INFORMATION=INFORMATION, ERROR=ERROR, QUESTION=QUESTION, CENTER=CENTER)
;
;END
