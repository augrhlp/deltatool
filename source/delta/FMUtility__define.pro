...
FUNCTION FMUtility::coordLatLonToLambert, xy, pars

  return, destXY
  
END

FUNCTION FMUtility::convertCoordSystemCode, origCode
...
FUNCTION FMUtility::doProjection, projInfo, xS, yS
...
FUNCTION FMUtility::getprojNum, name
...
FUNCTION FMUtility::getGraphicBackgroundColor

  return, getColor(235,235,235)
  
END

FUNCTION FMUtility::getGraphicPenColor

  return, getColor(100,100,100)
  
END

FUNCTION FMUtility::getCommentLabelFont

  return, "TIMES*20*ITALIC"
  
END

FUNCTION FMUtility::getStandardLabelFont

  return, "COURIER*8"
  
END

FUNCTION FMUtility::getStandardTextFont
...
FUNCTION FMUtility::colorObjLong64, colorObjs
...
FUNCTION FMUtility::colorObjToByte, colorObjs
...
FUNCTION FMUtility::updateColorTable, limits, leaveColor=leaveColor, colors=colors
...
FUNCTION FMUtility::buildStandardScaleClasses, limits, colors
...
FUNCTION FMUtility::getMultiPlotPosition, index, viewNumber, charsize=charsize, labelPos=labelPos
...
FUNCTION FMUtility::buildAxisTickMark, xData

  return, strtrim(xData, 2)
  
END

FUNCTION FMUtility::buildDateAxisTickMark, startDate, frequency, frameNumber, YEAR=YEAR
...
FUNCTION FMUtility::buildBestDateAxisTickMark, startDate, frequency, frameNumber
...
FUNCTION FMUtility::getColorTable, refValue, valueTable, colorTable, TEST=TEST
...
FUNCTION FMUtility::getOverPlotColor, index
...
FUNCTION FMUtility::overPlotGraphCheck, title, xTitle, yTitle, dataXLabel, measureUnit, dataNum, controlWindow, colorToUse=colorToUse
...
FUNCTION FMUtility::getColor, red, green, blue, indexes=indexes, THREE=THREE
...
FUNCTION FMUtility::IsFileNameCompatible, aString

  ;testString=byte(aString)
  invalidCharList=["!", "@", " ", "#", "$", "*"]
  for i=0, n_elements(invalidCharList)-1 do if (strpos(aString, invalidCharList[i]))[0] ne -1 then return, 0
  if strlen(strcompress(aString, /REMOVE)) eq 0 then return, 0
  return, 1
  
END

FUNCTION FMUtility::IsNumber, aString
...
FUNCTION FMUtility::buildWithAsterisk, list, prefix=prefix, numbers=numbers
...
FUNCTION FMUtility::buildFileDataStream, dataName, datalist, sepChar, blockSepChar, POINTER=POINTER
...
PRO FMUtility::convertStreamDataFile, record, dataName, datalist, sepChar, blockSepChar, POINTER=POINTER
...
FUNCTION FMUtility::getSysTime, FILECOMPATIBILITY=FILECOMPATIBILITY
...
FUNCTION FMUtility::calcDayOfYear, date, outHour=outHour
...
FUNCTION FMUtility::getDayNumberOfYear, year
...
FUNCTION FMUtility::checkDate, year, month=month, day=day, hour=hour, minute=minute, second=second, julNumber=julNumber
...
FUNCTION FMUtility::getDate, startDate, frequency, frameIndex
...
FUNCTION FMUtility::formatDate, date, template=template
